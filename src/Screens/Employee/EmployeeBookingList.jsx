...
import { getBooking, addBooking, updateBooking, checkoutBooking} from "../../apis/apibooking";
...
const EmployeeBookingList = () => {
  ...
  const handleSaveBooking = async (bookingArray) => {
    try {
  ...
  const handleCheckoutBooking = async (maDatPhongToCheckout) => {
    ...
    try {
      // Find the booking object to get maPhong for the checkoutBooking API
      const bookingToCheckout = bookingData.find(b => b.maDatPhong === maDatPhongToCheckout);

      // Validate if booking and its maPhong are found
      if (!bookingToCheckout || typeof bookingToCheckout.phongDichVus?.[0]?.maPhong === 'undefined' || bookingToCheckout.phongDichVus?.[0]?.maPhong === null) {
        throw new Error("Không tìm thấy thông tin phòng hoặc mã phòng cho đặt phòng này.");
      }

      const checkoutData = {
          maPhong: bookingToCheckout.phongDichVus[0].maPhong, // Get maPhong from the first phongDichVu
          maDatPhong: maDatPhongToCheckout
      };
      
      // Call the checkoutBooking API
      await checkoutBooking(checkoutData); 
  ...
  const filteredBooking = bookingData.filter((booking) => {
    ...
    // Check if the search text is included in staff's full name
